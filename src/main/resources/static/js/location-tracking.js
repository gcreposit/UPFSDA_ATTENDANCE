...
// Load data from backend instead of using mock data
async function loadMockData() {
    try {
        // 1. Fetch users data from backend
        ...
        // 2. Fetch location history from backend
        const locationResponse = await fetch('/api/data/location-history');
        if (!locationResponse.ok) {
            throw new Error(`Error fetching location history: ${locationResponse.statusText}`);
        }
        const locationData = await locationResponse.json();

        // Convert timestamps in location history to Date objects
        locationHistory = locationData.map(item => ({
            ...item,
            timestamp: new Date(item.timestamp)
        }));

        // Link each user with their last known location and set defaults
        mockUsers = usersData.map(user => {

            const lastKnown = locationHistory.find(h => h.username === user.username);
            return {
                ...user,
                isActive: lastKnown?.isActive ?? false,
                distanceTraveled: 0, // or calculate if you want
                currentLocation: lastKnown ? {
                    lat: lastKnown.location?.lat ?? lastKnown.lat,
                    lng: lastKnown.location?.lng ?? lastKnown.lon
                } : null,
...
function updateLocationHistory() {
    ...
    let filteredHistory = [...locationHistory].sort((a, b) => b.timestamp - a.timestamp);
    ...
    const duration = filterDurations[filter] ?? filterDurations["24h"];
    filteredHistory = filteredHistory.filter(h => (now - h.timestamp) <= duration);

    // Build HTML list
    const html = filteredHistory.slice(0, 10).map(item => {
        const lat = item.location?.lat ?? item.lat ?? null;
        const lng = item.location?.lng ?? item.lon ?? null;
        ...
        return `
            <div class="history-item ${item.isActive ? 'active' : 'inactive'}" 
                ...
                <div class="history-location">
                    ${lat && lng ? `${lat.toFixed(4)}, ${lng.toFixed(4)}` : 'Unknown'}
                </div>
            </div>
        `;
    }).join('');
